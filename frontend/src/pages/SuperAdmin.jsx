...
import { IconMail, IconSend, IconShield, IconRefresh } from '../Icons.jsx'
  ...
  return (
    <>
      <div className="row between mb">
        ...
        <span className="sm mut">Live · refreshes every 10s</span>
      </div>
      ...
      <div className="card mb">
        ...
        <p className="sm mut mb">
          OpenAI key is managed <b>only here</b> — not in .env, not in any other page.
          Encrypted at rest with <b>AES-GCM</b> (key derived from JWT_SECRET); masked in the UI so no
          one can see the raw value. Used immediately, no restart needed.
        ...
        {keys.map(k => (
          <div key={k.name} className="row between" style={{ padding: '12px 0', borderBottom: '1px solid var(--line)' }}>
            ...
            <div className="row" style={{ flex: 1, justifyContent: 'flex-end' }}>
              ...
              <button className="btn danger small" onClick={() => delKey(k.name)}>Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="card mb">
        ...
        {/* Agent status strip */}
        <div className="grid c4 mb" style={{ gap: 8 }}>
          {(monitor?.agents || []).map(a => (
            <div key={a.agent_id} className="card" style={{ padding: '10px 12px', borderColor: a.is_active ? 'var(--ok)' : '#fecaca' }}>
              <div className="row between">
                <b style={{ fontSize: 13 }}>{a.name}</b>
                <span className={`pill ${a.is_active ? 'ok' : 'gray'}`} style={{ fontSize: 10 }}>{a.is_active ? 'LIVE' : 'PAUSED'}</span>
              </div>
              <div className="sm mut" style={{ marginTop: 4 }}>
                ↑ {a.sent_today} sent · ↓ {a.recv_today} recv · {a.sent_last_1h}/hr
              </div>
              <div className="sm mut">Limit: {a.daily_limit}/day</div>
            </div>
          ))}
          {(!monitor?.agents || monitor.agents.length === 0) && <div className="empty">No agents yet</div>}
        </div>

        {monitor?.paused_agents?.length > 0 && (
          <div className="err mb sm">⚠ Paused agents: {monitor.paused_agents.join(', ')}</div>
        )}
        {monitor?.bounced?.total > 0 && (
          <div className="err mb sm">⚠ {monitor.bounced.total} bounce(s) detected — check Garbage for details</div>
        )}

        {/* Tab selector */}
        <div className="seg mb" style={{ marginBottom: 10 }}>
          {['outbound', 'inbound', 'bounced'].map(t => (
            <button key={t} className={monTab === t ? 'on' : ''} onClick={() => { setMonTab(t); setMonPage(1) }}>
              {t === 'outbound' ? `Outbound (${monitor?.outbound?.total ?? 0})`
               : t === 'inbound' ? `Inbound (${monitor?.inbound?.total ?? 0})`
               : `Bounced (${monitor?.bounced?.total ?? 0})`}
            </button>
          ))}
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table>
            <thead>
              <tr><th>When</th><th>Lead/From</th><th>Subject</th><th>Dir</th></tr>
            </thead>
            <tbody>
              {((monTab === 'outbound' ? monitor?.outbound?.items
                : monTab === 'inbound' ? monitor?.inbound?.items
                : monitor?.bounced?.items) || []).map(r => (
                <tr key={r.id}>
                  <td className="sm mut">{r.created_at ? new Date(r.created_at + 'Z').toLocaleString() : '—'}</td>
                  <td className="sm">{r.lead_email || r.from_addr || '—'}</td>
                  <td className="sm ellipsis" style={{ maxWidth: 260 }}>{r.subject || '(no subject)'}</td>
                  <td><span className={`pill ${r.direction === 'in' ? 'ok' : 'blue'}`}>{r.direction}</span></td>
                </tr>
              ))}
              {((monTab === 'outbound' ? monitor?.outbound?.items
                : monTab === 'inbound' ? monitor?.inbound?.items
                : monitor?.bounced?.items) || []).length === 0 && (
                <tr><td colSpan={4}><div className="empty">No {monTab} emails yet</div></td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {/* Pagination */}
        {(() => {
          const pages = monTab === 'outbound' ? (monitor?.outbound?.pages || 1)
                      : monTab === 'inbound' ? (monitor?.inbound?.pages || 1) : 1
          return pages > 1 ? (
            <div className="pagination mt">
              <button disabled={monPage <= 1} onClick={() => setMonPage(p => p - 1)}>‹</button>
              <span className="sm mut">Page {monPage} of {pages}</span>
              <button disabled={monPage >= pages} onClick={() => setMonPage(p => p + 1)}>›</button>
            </div>
          ) : null
        })()}
      </div>
